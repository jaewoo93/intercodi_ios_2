...
export default class Stage {
    constructor (div) {
        ...
        this.pages = [];
        this.pagesdiv = newDiv(this.div, 0, 0, 480, 100, {
            position: 'relative'
        });
        var me = this;
    ...
    sd () {
        var stg = gn('stage');
        var mask = newDiv(gn('codeStart'), stg.offsetLeft + 1, stg.offsetTop + 1, 482, 362,
            {
                position: 'absolute',
    ...
    sm (spr) {
        ...
        var mask = newDiv(gn('codeStart'), stg.offsetLeft + 1, stg.offsetTop + 1, w, h,
            {
                position: 'absolute',
                zIndex: ScratchJr.layerTop + 20,
                visibility: 'hidden'
